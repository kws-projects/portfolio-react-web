...
export const getDurationString = (fromDate: string, toDate?: string) => {
    ...
    if (!toDate) return `${fromDateObj.toLocaleString('default', { month: 'long' })}, ${fromDateObj.getFullYear()} - Present`
...
export const TimelineList = ({ items, className, children }: TimelineListProps) => {
    return (
    ...
    return (
        <>
            <div className="flex items-center w-full space-x-8">
                {item.image && 
                    <img className="w-20" src={item.image} alt="Company" />
                }
                ...
                <div>
                    ...
                    {item.fromDate && item.toDate && 
                        <p className="text-gray-500">{getDurationString(item.fromDate, item.toDate)}</p>
                    }
                        ...
                        <div className="flex items-center w-full space-x-8 mt-4 mb-4 pl-0 sm:pl-28" key={experience.id}>
                            ...
                            <div className="pb-2">
                                ...
                                {experience.fromDate && !experience.toDate &&
                                    <p className="text-gray-500">{getDurationString(experience.fromDate)}</p>
                                }

                                {experience.fromDate && experience.toDate && 
                                    <p className="text-gray-500">{getDurationString(experience.fromDate, experience.toDate)}</p>
                                }
